...
### Phase 8 — 멀티 에이전트 파이프라인 설계 (초안)
- 사용자가 데이터 수집/분석/오류 탐지 3개 에이전트로 파이프라인을 쪼개고 싶다고 제안 — 각각 새 세션에서 세밀하게 튜닝할 계획이라 먼저 설계 문서부터 작성
- 3개 제안을 재검토해 "코드로 이미 되는 부분"과 "Claude 판단이 필요한 부분"을 분리: (1) 데이터 수집은 주식 API 호출(코드)과 뉴스 신뢰도/편향 판단(에이전트)을 구분 (2) 분석 에이전트는 지금 헤드리스 브리핑과 거의 동일 — 외부 GitHub 분석 코드를 그대로 이식하는 건 "AI 판단은 Claude가 직접" 원칙과 충돌해서 비추천, 캔들/이동평균 등 가벼운 지표를 프롬프트에 추가하는 쪽을 제안 (3) "오류 탐지"는 재시도 로직(이미 코드로 존재)과 통계적 이상탐지(에이전트 필요)를 분리하고 이름을 "이상 탐지/모니터링"으로 좁힘
- 실행 방식 확인 질문: 자동화 파이프라인 통합 vs 대화형 서브에이전트 vs 둘 다 — 사용자가 "자동화 파이프라인 통합"(무인 실행) 선택
- [AGENTS.md](AGENTS.md) 작성 — 3단계(수집·검증 → 분석 → 이상탐지) 구조, 각 단계 입출력 JSON 스키마, 권한 스코프(기존 `run_briefing.sh`의 좁은 `--settings` 패턴 확장), 열린 질문(저신뢰 기사 제외 vs 표시, 브리핑 이력 보관 방식, 이상 알림 분리 여부, 3단계로 늘어난 Claude Pro 사용량) 정리
- PLAN.md에 Phase 8로 등록. 아직 구현 전 — 다음은 Stage 1(데이터 수집·검증)부터 새 세션에서 시작
- 사용자가 "2. 분석만 에이전트하면 되는거아닌가?" 질문 — 재검토 후 동의: 분석 에이전트는 어차피 뉴스를 전부 읽고 판단하므로 신뢰도 검증을 같은 호출에 통합하는 게 자연스럽고, 별도 호출로 나누면 얻는 것 없이 Claude Pro 사용량만 1회→3회로 늘어남. 이상 탐지는 여러 회차 히스토리를 보는 근본적으로 다른 입력이라 분리 유지, 대신 비용 억제 위해 하루 1회로 제한. **3단계 → 2단계(분석+이상탐지)로 설계 축소**, AGENTS.md/PLAN.md Phase 8 체크리스트 갱신
