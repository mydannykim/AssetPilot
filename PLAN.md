...
- [x] README 및 설정 가이드 작성 — 기능별로 계속 갱신해왔던 걸 "처음 한 번만 하는 설정" 체크리스트(env → init-db → 스냅샷 launchd → claude 로그인 → 브리핑 launchd → terminal-notifier)로 한 곳에 정리

## Phase 7 — (향후 별도 진행) 자동매매 확장
> 지금 단계에서는 설계만 메모. 실제 구현은 Phase 0~6 완료 후 사용자 요청 시 별도 기획. 사용자가 나중에 직접 진행하기로 함(08-19).
- [ ] 매매 전략 규칙 정의
- [ ] 리스크 한도(포지션 크기, 일일 손실 한도 등) 설계
- [ ] 주문 실행 전 승인/확인 절차 설계
- [ ] 실거래 전 모의투자(paper trading) 검증 단계

## Phase 8 — 멀티 에이전트 파이프라인 고도화
> 지금(Phase 4/5)의 단일 헤드리스 `claude -p` 분석을 역할별 에이전트 3단계로 쪼개는 작업.
> 상세 설계는 [AGENTS.md](AGENTS.md) 참고. 각 에이전트는 별도 세션에서 하나씩 튜닝 예정.
- [x] 설계 문서 작성 (AGENTS.md) — 3단계 구조, 입출력 스키마, 권한 범위, 열린 질문까지 정리
- [ ] Stage 1: 데이터 수집·검증 에이전트 — 뉴스 신뢰도/편향 판단, `news_curated.json` 출력
- [ ] Stage 2: 분석 에이전트 — 기존 브리핑 로직 재사용, 입력을 curated 뉴스로 교체
- [ ] Stage 3: 이상 탐지 에이전트 — 스냅샷/브리핑 히스토리 이상 징후 판단, `pipeline_health.json` 출력
- [ ] 파이프라인 통합 (`run_briefing.sh` 3단계 순차 호출로 재구성) + launchd 재검증
- [ ] 3단계로 늘어난 만큼 Claude Pro 사용량 재점검, 필요시 장중 빈도 재조정
