...
> 지금(Phase 4/5)의 헤드리스 `claude -p` 분석에 신뢰도 판단을 통합하고, 이상 탐지를 별도
> 에이전트로 추가하는 작업. 상세 설계는 [AGENTS.md](AGENTS.md) 참고.
- [x] 설계 문서 작성 (AGENTS.md) — 처음엔 3단계(수집·검증/분석/이상탐지)로 설계했다가, 수집·검증은
      어차피 분석 에이전트가 뉴스를 다 읽으므로 같은 호출에 통합하는 게 낫다고 판단해 2개로 축소
- [ ] 분석 에이전트: `run_briefing.sh` 프롬프트에 뉴스 신뢰도/편향 판단 기준 추가 (기존 스크립트 확장, 새 파일 없음)
- [ ] 이상 탐지 에이전트: 브리핑 이력 보관 방식 결정 → `run_anomaly_check.sh` 신규 구현 → `pipeline_health.json` 출력
- [ ] `com.assetpilot.anomaly` launchd 등록 (하루 1회) + 검증
- [ ] 늘어난 Claude Pro 사용량(하루 5회 → 6회) 재점검
